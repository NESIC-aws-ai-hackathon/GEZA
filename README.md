...
| 問い | 答え |
|------|------|
| **何を作るのか** | やらかした内容を **一言** 入れるだけで、台本生成 → AIアバター相手の練習 → **本番中もAIが耳打ち** まで全自動。使い続けると **AIなしでは謝れない人間** が完成する「謝罪丸投げコンシェルジュ」 |
| **なぜ人をダメにするのか** | ① **練習モード** でAIの台本を読み上げるだけの練習を繰り返すうちに、自分の言葉で謝る力が退化する。② **耳打ちモード** で本番中もAIが「それ言ったらアウト」と囁き続け、AIの指示なしでは怖くて口が開けなくなる。**AIなしでは謝れなくなる――それがGEZAの「人をダメにする」設計** |
| **何が動くのか** | U0〜U4 実装・デプロイ完了（MVP P0 全機能稼働中）。予選（5/30）までに練習モード（U3）+ 耳打ちモード（U9）を含むフルデモを実装予定 |
...
台本生成 → AIアバター相手の練習 → **本番中のリアルタイム耳打ち** まで全部AIがやる謝罪丸投げコンシェルジュです。  

**練習モード** でAIの台本を繰り返し読み上げるうちに、自分の言葉で謝る力が退化する。  
**耳打ちモード** で本番中もAIが「それ言ったらアウト」と囁き続け、AIの指示なしでは怖くて口が開けなくなる。  

使えば使うほど **AIなしでは謝れない人間** が出来上がる——これがGEZAの「人をダメにする」設計です。

### Human-in-the-Loop な謝罪支援

GEZAは謝罪を完全自動化するサービスではありません。  
**AIが謝罪を設計し、人間が最後に謝る** ——これがGEZAの Human-in-the-Loop です。  
そしてその「謝る」瞬間すら、AIが耳元で囁いています。

> **注**: GEZAにおける Human-in-the-Loop は「AIが設計・最適化し、人間が実行する」という独自解釈です。一般的な HITL（人間がAIを監督・修正する）とは逆の構造を意図的に採用し、「謝るのは人間の責任」という倫理的境界を明確に残しています。
...
│                                                           │
│   【謝罪前】           【謝罪中】          【謝罪後】       │
│   AI が設計する        人間が実行する      AI が振り返る     │
│                                                           │
│   角度アセスメント     怒り残量スキャナー   返信分析          │
│   相手分析・台本生成   GEZA耳打ちモード    カルテ蓄積        │
│   リハーサル           NG発言リアルタイム検知 傾向診断        │
│   送る前チェック                           再発防止策        │
│                                                           │
│   ───────── AI が担う ─────────┬──────── AI が担う ──────── │
│                                │                           │
...
謝罪という行為において、AIが介入できない最後の一線——それは **頭を下げること** です。  
GEZAは謝罪の前後をAIで包囲しながら、「詫びる」という人間の責任だけを残します。  
**使い続けるほど、謝罪のうち「自分で考えた部分」はゼロに近づいていきます。**

GEZAは単発の謝罪文生成ツールではありません。  
台本生成から練習、本番中の耳打ち、相手からの返信分析、次の一手の提案、謝罪カルテへの記録までを一貫して支援する謝罪コンシェルジュです。

**最終的にユーザーは、AIなしでは「ごめんなさい」の一言すら怖くて言えない人間になります。**
...
★ キラー機能① — 練習モード（AIなしでは口が開けなくなる体験）
  AIアバターとの謝罪リハーサル ← AI製台本を繰り返し読み上げるうちに「自分の言葉」が消える
  200感情×15カテゴリの怒りアバター ← 怒り度ゲージが動き、NGワード検知で激怒、許されたらクリア
  失敗してもAIが台本を無限に書き直す ← ユーザーは反省も工夫も不要。ただ読むだけ

★ キラー機能② — GEZA耳打ちモード（AIなしでは怖くて謝れなくなる体験）
  怒り残量スキャナー    ← 相手の発言から怒り残量・失望度・許容余地・反論危険度をリアルタイム推定
  GEZA耳打ちモード      ← ユーザーの発話から言い訳・逆ギレ・責任転嫁・NGワードを検知し短い助言を返す
                           対面モード: イヤホン越し / Web会議モード: 専用パネル

継続支援（怒られる前から、許されるまで）— 謝罪後
...
### 軸 2：練習モード —— AIの台本なしでは口が開けなくなる【★キラー機能①】

練習モードは「練習して上手くなる」ためのものではありません。  
...
**「自分の言葉で謝る」練習は一度も行われません。** AIの台本を繰り返し読み上げるうちに、台本がないと「何を言えばいいかわからない」状態になります。  
これが **「AIなしでは謝れない」依存の第一歩** です。

> *「10回練習しました。でも、一度も自分でセリフを考えたことはありません。次の謝罪も、GEZAなしでは口が開けません。」*

### 軸 3：謝罪のエンタメ化

GEZA では、謝罪相手は「謝罪ボス」として登場し、感情 200 種類（15カテゴリ）で反応し、怒り度ゲージが動き、
...
### 軸 5：GEZA耳打ちモード —— AIなしでは怖くて謝れなくなる【★キラー機能②】

GEZAの本質は「AIが設計し、人間が詫びる」 Human-in-the-Loop 構造です。  
しかし、「詫びる」瞬間すら、GEZAは人間を一人にしません。

- **怒り残量スキャナー** — 相手の発言をリアルタイムで分析し、怒り残量・失望度・許容余地・反論危険度を推定。「まだ謝り足りていない」をAIが教える
- **GEZA耳打ちモード** — ユーザー自身の発話を監視し、言い訳・逆ギレ・責任転嫁・NGワードを検知。「それ言ったらアウト」を短い助言で耳元に返す

**謝罪中、あなたが口を開くたびにGEZAが見張っている。** 頭を下げている最中すら、自分の判断ではなくAIのガイダンスに従う。  

その結果、ユーザーは **「AIの耳打ちがないと、余計な一言を言ってしまいそうで怖い」** と感じるようになる。  
AIのガードレールなしでは怖くて口が開けない——これが **「AIなしでは謝れない」依存の完成形** です。

> *「謝罪の前も、最中も、後も、GEZAがいる。あなたが自分で考えた瞬間は、ゼロです。」*
...
### ApologyMeter プロトタイプ（謝罪角度アセスメント）

謝罪角度測定・印鑑スタンプ演出・SE音の実動検証プロトタイプ。

| 項目 | 内容 |
|---|---|
| 実装 | `prototype/apology-meter.html` (Pure HTML/CSS/JS) |
| 演出 | 実印風朱肉スタンプ + SVG feTurbulenceかすれ + ゾーン別色変化 |
| SE音 | Web Audio API合成：重低音「ドン！」音（6レイヤー）+ ゴゴゴゴ地響き持続音（100°+） |
| パーティクル | 角度連動：0° = 4個、180° = 80個以上（多段ウェーブ） |
| 演出エスカレーション | 45°+: 揺れ+衝撃波 / 75°+: バウンス着地2発目ドン / 100°+: 重揺れ+ゴゴゴゴ / 150°+: カオスドン連打+光芒 / 180°: 全面崩壊 |

![ApologyMeter デモ](prototype/videos/謝罪角度アセスメント/ApologyMeter.gif)

動画: [prototype/videos/謝罪角度アセスメント/ApologyMeter .mp4](prototype/videos/謝罪角度アセスメント/ApologyMeter%20.mp4)
...
| **③** | [stories.md](aidlc-docs/inception/user-stories/stories.md) | 5分 | INVEST済41ストーリー/271SP |
...
| 審査観点 | 主要参照先 | 補足 |
|--------|----------|---------|
| **Intent（意図・テーマ適合性）** | このREADME「ハッカソンテーマとの適合性」セクション / [requirements.md](aidlc-docs/inception/requirements/requirements.md) | 「人をダメにする」5軸の構造を詳述 |
| **Unit 分解（開発計画）** | [unit-of-work.md](aidlc-docs/inception/application-design/unit-of-work.md) / [unit-of-work-dependency.md](aidlc-docs/inception/application-design/unit-of-work-dependency.md) | U0〜U9・271SP・依存関係・実装順定義済み |
| **創造性** | このREADME「キラー機能」/ [application-design.md](aidlc-docs/inception/application-design/application-design.md) | ①練習モード（AI台本依存→自分で謝れなくなる）②GEZA耳打ちモード（本番中もAI依存→怖くて謝れなくなる）。「AIなしでは謝れない」体験を実現する既存ツールに存在しない設計 |
...
### キラー機能：「AIなしでは謝れない」体験を生み出す二大機能

**練習モード** と **GEZA耳打ちモード** ——この2つがGEZAのキラー機能です。

| 機能 | 何をするか | なぜ「人をダメにする」のか |
|------|------------|---------------------------|
| **① 練習モード** | AIが作った台本をAIアバター相手に読み合わせる | AIの台本なしでは「何を言えばいいかわからない」状態になる |
| **② 耳打ちモード** | 本番中にAIがリアルタイムで耳元に助言 | AIのガードなしでは「余計な一言を言いそうで怖い」状態になる |

練習で「AIの台本がないと口が開けない」、本番で「AIの耳打ちがないと怖くて謝れない」。  
この2つが揃って初めて、**「AIなしでは謝れない人間」が完成します。**

---

### 謝罪角度アセスメント（ApologyMeter）

やらかしの内容を入力するだけで、AI が謝罪の深刻度を **0〜180° の角度** で即座に数値化。  
**6ゾーン×14段階** のスケールで、実印風の朱肉スタンプ演出＋重低音「ドン！」衝撃音＋ゴゴゴゴ地響きで可視化。  
AI 判定 vs 自己申告のギャップ分析で「自分が思うより相手はもっと怒っている」ことに気づかせます。

```
🟢 日常 (0°〜30°)     : #0 直立不動(0°) → #1 目礼(15°) → #2 会釈(30°)
🟡 ビジネス (45°〜60°) : #3 敬礼(45°) → #4 最敬礼(60°)
🟠 危機 (75°〜90°)     : #5 深謝(75°) → #6 直角のお辞儀(90°)
🔴 覚悟 (100°〜120°)   : #7 土下座(100°) → #8 土下座プレス(110°) → #9 寝下座(120°)
⚫ 超越 (135°〜150°)    : #10 這い寝下座(135°) → #11 焦げ寝下座(150°)
✨ 昇天 (165°〜180°)    : #12 焼き寝下座(165°) → #13 炭化寝下座(180°)
```

---

## 謝罪ライフサイクル支援：謝罪前・謝罪中・謝罪後

GEZAは、謝罪の一瞬だけを支援するアプリではありません。  
謝罪前・謝罪中・謝罪後のやり取りを継続的に支援し、ユーザーごとの謝罪カルテとして蓄積します。  
**AIが介入できない唯一のポイントは「頭を下げる」瞬間だけ。それ以外はすべてGEZAが担います。**
...
### 2. 謝罪中支援：怒り残量スキャナー＋GEZA耳打ちモード

謝罪の本番中にAIがリアルタイムで支援する、GEZAのキラー機能②です。

#### 怒り残量スキャナー
相手の発言をリアルタイムで分析し、以下を推定・表示します。
- **怒り残量**（0〜100%）— まだ怒っているか
- **失望度** — 怒りではなく「がっかり」に変化していないか
- **許容余地** — 許す余地がどのくらい残っているか
- **反論危険度** — 相手が反論・追撃してくる可能性

#### GEZA耳打ちモード
ユーザー自身の発話をリアルタイムで監視し、危険な発言を検知して短い助言を返します。
- **言い訳検知** → 「それ、言い訳に聞こえます」
- **逆ギレ検知** → 「逆ギレ危険！深呼吸してください」
- **責任転嫁検知** → 「他人のせいにしていませんか？」
- **NGワード検知** → 「その言葉はNGです。言い換えて」

GEZA耳打ちモードは2つのモードで動作します。

| モード | 入力ソース | 助言の出し方 | 想定シーン |
|--------|-----------|------------|----------|
| **対面モード** | スマホマイク（イヤホン装着） | イヤホン越しの短い音声助言 | 対面謝罪・訪問謝罪 |
| **Web会議モード** | PC音声出力＋マイク入力 | 専用タブ/前面パネルに短文表示 | Zoom / Teams / Google Meet |

> **対面モード**: イヤホン越しに短い助言を返す。将来的にはARグラス連携も想定。  
> **Web会議モード**: 会議相手にAI助言を聞かせず、画面上で静かに支援する。助言は音声ではなく短文テキストで表示し、会議の邪魔にならないUI設計とする。

AIが整え、人間が向き合う。GEZAは、対面でもWeb会議でも、謝罪中の余計な一言を防ぎます。
...
| リアルタイム反応 | なし | なし | 人間講師 | **200感情AIアバター** |
...
| 音声合成 | Amazon Polly（女性: Kazuha / 男性: Takumi, ja-JP, Neural）+ SpeechMarks Viseme 口パク同期 |
| 音声認識 | Amazon Transcribe Streaming（WebSocket 直接接続, ja-JP）|
| バックエンド | AWS Lambda（Python 3.12, 512MB）× 21 関数 |
...
| API | API Gateway HTTP API v2（JWT Authorizer, 21 エンドポイント）|
...
```
U0: 共通インフラ + FEコアモジュール（SAMデプロイ）               ✅ 完了
U1: トップ画面 + Cognito認証                                    ✅ 完了
U2: コンシェルジュコア（謝罪角度アセスメント + 台本フル生成）     ✅ 完了
U3: 練習モード（AI製台本の読み合わせ + 200感情アバター）         ✅ 完了 ★キラー機能①
U4: 謝罪後支援 + カルテ（再発防止策・フォローメール・謝罪履歴）  ✅ 完了
U9: GEZA耳打ちモード（怒り残量スキャナー + リアルタイム助言）     🔧 実装中 ★キラー機能②
```

**U0〜U4 で「入力一つ→台本完成→練習→振り返り」の丸投げ体験が完成。**  
U9 の耳打ちモードで「本番中もAI依存」を加え、**「AIなしでは謝れない」体験を完成させます。**
...
- **AIアバター相手のリハーサル体験 → 台本なしでは口が開けなくなる依存構造**
- 音声・表情・怒り度を含むロールプレイ体験
- 結果の保存と振り返り
- **本番中のリアルタイム耳打ち → AIなしでは怖くて謝れなくなる依存構造**
...
| スコープ | ユニット | SP | 目標 | フェーズ |
|---------|---------|:--:|-----|---------|
| **丸投げ体験** | U0+U1+U2 | 73+基盤 | 「入力→台本完成」が動く | ✅ 完了 |
| **練習で依存** | U0〜U3 | 124+基盤 | AIの台本なしでは口が開けなくなる | ✅ 完了 |
| **振り返りで依存** | U0〜U4 | 152+基盤 | 反省すらAIに丸投げ | ✅ 完了 |
| **★ 本番で依存** | U0〜U4+U9 | 194+基盤 | **AIなしでは怖くて謝れなくなる** | **🔧 予選MVP** |
| **P1完全体** | +U5+U6 | 230+基盤 | ストーリー・上司モード | 決勝 |
| **継続支援** | +U7+U8 | **271** | 継続的謝罪コンシェルジュ | 決勝 |

> **予選での訴求ポイント**: U9（耳打ちモード）は **予選の主要デモ機能**。  
> 練習モード（U3）＋耳打ちモード（U9）の組み合わせで「AIなしでは謝れない」を体験させることが予選デモの核。

### ハッカソンタイムライン

| フェーズ | 期間 | 完成ユニット | 成果物 |
|------|------|------------|------|
| **書類審査** | 完了 | U0〜U4 完了（P0 MVP稼働中） | README + aidlc-docs + デプロイ済みアプリ |
| **予選（5/30）まで** | 残り3週間 | **+U9（耳打ちモード）** | 「AIなしでは謝れない」完全体験デモ |
| **予選後〜決勝** | +時間が許す限り | +U5→+U6→+U7→+U8 | ストーリーモード + 継続的謝罪コンシェルジュ |

> **開発進捗**: U0〜U4 は実装・デプロイ・スモークテスト完了済み。予選までに U9（耳打ちモード）を実装し、練習モード×耳打ちモードの組み合わせデモを予選のキラーデモとする。
...
ブラウザ
  ...
  └── 共通モジュール: AvatarController, EmotionDefs, StateManager, ApiClient
       ...
       HTTP API v2
              │
        Lambda × 21
          Nova Lite系: assess-apology, evaluate-apology, analyze-karte, analyze-anger, detect-danger-speech
          Sonnet系: generate-opponent, generate-plan, generate-feedback, analyze-reply ...
              │
...
AIは**カテゴリID**を返却し、FEがカテゴリ内の感情をランダムに選択・自然な表情揺らぎを実現。

`fierce_anger(16) → anger(14) → intimidation(12) → irritation(14) → sadness(14) → contempt(14) → surprise(12) → suspicion(12) → resignation(12) → confusion(14) → interest(14) → relief(14) → acceptance(14) → gratitude(12) → forgiveness(12) = **200種類**`  
特殊エフェクト：rage → 画面揺れ+赤フラッシュ、forgiveness → 画面が明るくなる
カテゴリ内ランダム遷移：2〜4秒間隔で同一カテゴリ内の別感情に自然変化（CSS transition 200ms 補間）
...
INCEPTION PHASE
  ...
  └── 6. Units Generation          → aidlc-docs/inception/application-design/
          ├── unit-of-work.md           （10ユニット定義(U0-U9)・ディレクトリ構成・完了基準）
          ├── unit-of-work-dependency.md（依存マトリックス・実装順フロー）
          └── unit-of-work-story-map.md （全41ストーリー → ユニット マッピング）
```
...
| U9 | 謝罪中支援（怒り残量スキャナー・耳打ち） | E10 | 42 | 6（★予選）|
| | **合計** | | **271** | |
...
| facesjs SVG アバター視覚感情表現 | CSS transform で **200 感情（15カテゴリ）**を実装確認（フォーク版） | ✅ 検証済み | MP4 動画案を廃棄し SVG 方式を採用 |
| LLM 感情ラベル分類（プロトタイプ） | Nova Lite が **5 感情**ラベルを返却（怒り/苛立ち/失望/驚き/納得） | ✅ 検証済み | 本実装では 15カテゴリ（200 感情）マッピングに拡張予定 |
...
| **ApologyMeter UI演出** | Web Audio API 6レイヤードン音＋ゴゴゴゴ地響き・CSS変数ゾーン色・多段エフェクト | ✅ 検証済み | 外部ライブラリ不使用で全演出成立。Pure HTML/CSS/JS 採用確定 |
...
**練習モード（U3）** と **GEZA耳打ちモード（U9）** をキラー機能と位置づけ、予選デモの核に据えました。  
練習モードで「AIの台本なしでは口が開けない」、耳打ちモードで「AIのガードなしでは怖くて謝れない」——  
この2つが揃うことで「AIなしでは謝れない人間」が完成する設計です。  
ApologyMeter は丸投げの入口として重要ですが、キラーは「依存を生む」上記2機能です。
...
│       ├── avatar.js         # AvatarController（facesjs + 200感情・15カテゴリ）
...
⑤ AWS フルサーバーレス × 日本語特化
   → Polly Kazuha（女性）/ Takumi（男性）（日本語 Neural TTS）+ Transcribe 日本語ストリーミング
...
⑦ 謝罪中リアルタイム支援（怒り残量スキャナー・GEZA耳打ちモード）★キラー機能②
   → 謝罪の本番中にAIがリアルタイムで伴走する唯一の体験
   → 「AIが考え、人間が詫びる」を究極まで推し進めた Human-in-the-Loop
   → AIのガードなしでは怖くて謝れなくなる依存体験
```
...
[フィードバック]
  ...
  └→ カルテに保存（謝罪履歴・スコア推移）
       ↓
[耳打ちモード（★キラー機能②）]
  本番謝罪中 → 怒り残量スキャナーが相手の感情をリアルタイム推定
  危険発言検知 →「言い訳に聞こえます」「逆ギレ危険！」と耳打ち
  └→ AIのガードなしでは怖くて謝れなくなる
```
...
|---------|:----:|
| INCEPTION | ✅ 完了・承認済み |
| CONSTRUCTION | U0〜U4 完了・デプロイ済み / U5 設計中 / U9 予選向け実装予定 |
