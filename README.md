...
| 問い | 答え |
|------|------|
| **何を作るのか** | やらかした内容を **一言** 入れると、謝罪の深刻度判定 → 相手分析 → 台本全文生成 → タイミング指示まで **AI が全自動で出す**。あなたは頭を下げるだけ。さらに、謝罪中のリアルタイム支援（怒り残量スキャナー・GEZA耳打ちモード）、送る前の文面チェック、相手の返信分析、謝罪カルテへの蓄積、謝罪傾向診断まで **謝罪前・謝罪中・謝罪後** を一貫して丸投げできる「謝罪丸投げコンシェルジュ」 |
| **なぜ人をダメにするのか** | 謝罪という「自分で考える行為」を完全代行する。使い続けるほど「自分で誠意を形にする力」が静かに失われ、相手の怒りや自分の反省パターンすらAIに読ませるようになる設計。**AIが考え、人間が詫びる――Human-in-the-Loop な謝罪のかたち** |
...
謝罪の深刻度判定 → 相手分析 → 台本生成 → タイミング指示まで **全部 AI が勝手にやる** 謝罪丸投げコンシェルジュです。  
**AIが整え、人間が向き合う**——謝罪で失敗するリスクを下げ、関係修復に集中できる環境をAIが構築します。  
裏を返せば、使い続けるほど「自分で謝り方を考える力」が失われていく——人をダメにするサービスです。
...
> **位置づけ**: リハーサルは「入力→台本→丸投げ」という核心体験（U2）に追加できるオプション機能（U3）です。U2 コアのみで丸投げ体験は完結します。
...
- 将来的にはイヤホンやARグラスでのリアルタイム支援を想定

**謝罪中、あなたが口を開くたびにGEZAが見張っている。** 頭を下げている最中すら、自分の判断ではなくAIのガイダンスに従う。  
「最後に残された人間の領域」すらAIが侵食する。これがGEZAの決勝構想です。
...
**6ゾーン×14段階** で表現し、**ライトモードのスマホ縦長画面**上で、**実印風の朱肉スタンプ演出（白い紙面＋二重輪＋かすれ表現）＋重低音「ドン！」衝撃音＋ゴゴゴゴ地響き** で可視化。  
...
### 2. 謝罪中支援：怒り残量スキャナー＋GEZA耳打ちモード（決勝構想）

謝罪の本番中にAIがリアルタイムで支援する、GEZAの決勝向け拡張構想です。
...
U9: 謝罪中支援（怒り残量スキャナー・GEZA耳打ちモード）   ← 決勝拡張
```
...
|---------|---------|:--:|-----|---------|
| **最低限（丸投げ体験）** | U0+U1+U2 | 73+基盤 | 「入力→台本完成」が動く | **予選MVP** |
| **デモ成立** | U0+U1+U2+U3 | 124+基盤 | リハーサル付きフルデモ | 予選 |
| **MVP完全体** | U0〞U4 | 152+基盤 | 予選会アピール | 予選 |
| **P1完全体** | U0〞U6 | 188+基盤 | 上司モードまで全実装 | 決勝 |
| **継続支援** | U0〞U8 | 229+基盤 | 継続的謝罪コンシェルジュ | 決勝 |
| **謝罪中支援** | U0〞U9 | **271** | 謝罪ライフサイクル完全体 | **決勝拡張** |

> **決勝向け拡張構想**: U9（謝罪中支援）は決勝デモのインパクトを最大化する拡張。  
> 怒り残量スキャナーとGEZA耳打ちモードにより、「謝罪中もAIがループから逃がさない」体験を実現。  
> Inception段階ではユーザー体験・要求として整理し、具体的なインフラ設計には踏み込まない。
...
| **予選後〜決勝** | +時間が許す限り | +U3→+U4→+U9（謝罪中支援） | フルスコープ + 怒り残量スキャナー・耳打ちモード |

> **リスク認識**: 全体はハッカソン期間内の完成を約束していません。予選までに U0+U1+U2 (謝罪コンシェルジュ、謝罪角度アセスメント)を動かすことを最優先とし、U3以降はボーナスと位置づけます。U9（謝罪中支援）は決勝デモの差別化要素として構想。
...
| U9 | 謝罪中支援（怒り残量スキャナー・耳打ち） | E10 | 42 | 10（決勝）|
| | **合計** | | **271** | |
...
⑦ 謝罪中リアルタイム支援（怒り残量スキャナー・GEZA耳打ちモード）【決勝構想】
