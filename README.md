...
| 問い | 答え |
|------|------|
| **何を作るのか** | やらかした内容を **一言** 入れると、謝罪の深刻度判定 → 相手分析 → 台本全文生成 → タイミング指示まで AI が全自動で出す「謝罪行動支援AI」。さらに、謝罪中のリアルタイム支援（怒り残量スキャナー・GEZA耳打ちモード）、送る前の文面チェック、相手の返信分析、謝罪カルテへの蓄積、謝罪傾向診断まで **謝罪前・謝罪中・謝罪後** を一貫して支援する。表向きの価値は「謝罪失敗リスクを下げ、人間が関係修復に向き合える状態をAIが整える」こと |
...
謝罪の深刻度判定 → 相手分析 → 台本生成 → タイミング提案まで **全部 AI が勝手にやる** 謝罪行動支援AIです。  
...
| バックエンド | AWS Lambda（Python 3.12, 512MB）× 20 関数 |
...
| API | API Gateway HTTP API v2（JWT Authorizer, 20 エンドポイント）|
...
|---------|---------|:--:|-----|---------|
| **最低限（丸投げ体験）** | U0+U1+U2 | 79 | 「入力→台本完成」が動く | **予選MVP** |
| **デモ成立** | U0+U1+U2+U3 | 116 | リハーサル付きフルデモ | 予選 |
| **MVP完全体** | U0〜U4 | 144 | 予選会アピール | 予選 |
| **P1完全体** | U0〜U6 | 180 | 上司モードまで全実装 | 決勝 |
| **継続支援** | U0〜U8 | 221 | 継続的謝罪コンシェルジュ | 決勝 |
| **謝罪中支援** | U0〜U9 | 245+ | 謝罪ライフサイクル完全体 | **決勝拡張** |
...
| U9 | 謝罪中支援（怒り残量スキャナー・耳打ち） | E10 | 24 | 10（決勝）|
| | **合計** | | **245** | |
