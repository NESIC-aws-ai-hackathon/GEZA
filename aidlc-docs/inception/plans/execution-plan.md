...
|-----------|----------|------|------------|------|----|:------:|
| **U0** | 共通インフラ + FEコアモジュール | - | SAM（14 Lambda スタブ + Cognito + APIGW + DynamoDB + S3 + CloudFront） + FE共通モジュール | なし | (基盤) | 最高 |
...
| **U3** | 謝罪練習シミュレーション | E4 | Lambda（evaluate-apology/text-to-speech） + フロントエンド（アバター+入力UI+ApologyMeter） | U0, U1, U2 | 51 | P0 |
...
| **U6** | 上司モード | E7 | Lambda（evaluate-guidance/generate-guidance-feedback） + フロントエンド（上司練習UI） | U0, U1, U3 | 23 | P1 |
