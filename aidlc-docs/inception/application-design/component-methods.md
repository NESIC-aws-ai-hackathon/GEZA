...
6ゾーン×14段階の謝罪角度メーター。ライトモードのスマホ縦長UIで、実印風の朱肉スタンプ演出（白い紙面、二重輪、かすれ表現）＋ゾーン別SE音（14種類）を表示する。
...
```javascript
class ApologyMeter {
  // 判子スタイルスタンプ・ピクトグラム・SE音を指定コンテナに初期描画
  init(containerEl)

    // 謝罪角度をセットし、実印風朱肉スタンプ演出（白い紙面＋二重輪＋かすれ）＋ゾーン別SE音で表示
  // degree: 0〜180 (#0 直立不動〜#13 炭化寝下座)
  setDegree(degree)
  ...
  // AIアセスメント値とユーザー自己申告値のギャップを視覚的に表示
