...
| 5 | 感情ラベル精度 | ✅ OK | **5感情分類**（怒り/苛立ち/失望/驚き/納得）を Nova Lite で検証 |
| 6 | NGワード検知 | ✅ OK | 「次から気をつけます」等を正確検知 |
| 7 | 会話連続性（3ターン以上） | ✅ OK | 最大6ターン履歴保持 |
| 8 | facesjs SVGアバター | ✅ OK | 瞬き・視線・うなずき・発話モーション |

> **屠感の分離**: プロトタイプは **5 感情ラベル**（Nova Lite 出力）で検証。本実装では facesjs フォーク版（v5.0.3）を用いて **200 感情視覚表現（15カテゴリ）**に拡張する。
> **LLM 層の分業**: リアルタイム評価・分類は **Nova Lite**（検証済み）、台本・相手分析等高品質生成は **Claude Sonnet 4.5**（本実装予定）。
...
│   ├── style.css             # スタイル（スマホ375px基準）
    └── app.js                # フロントエンドロジック（facesjs v3 → 本実装は v5.0.3 フォーク）
...
`prototype/videos/` に検証時の動作録画を配置しています。

| ファイル名 | 内容 |
|-----------|------|
| `GEZA_prototype_1.gif` | テキスト入力でリアルタイム会話が進行する様子（GIFアニメ） |
| `GEZA_prototype_2.gif` | 感情による表情変化の様子（GIFアニメ） |
| `GEZAプロトタイプ_1.mp4` | 上記1の音声付き原版 |
| `GEZAプロトタイプ_2.mp4` | 上記2の音声付き原版 |

> **注**: プロトタイプのアバター表現は facesjs SVG + CSS transform で実装しており、MP4動画切り替え方式は採用していません。これらの動画は技術検証の記録です。
